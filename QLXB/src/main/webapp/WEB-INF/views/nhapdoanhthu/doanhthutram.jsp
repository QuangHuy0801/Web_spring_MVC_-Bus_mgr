...
<body>
  ...
  <div class="collapse navbar-collapse" id="navbarTogglerDemo01">
    <a class="navbar-brand " href="themdoanhthutram/${idDTN}.htm"> <button type="button" class="btn btn-success"
																data-dismiss="modal">Thêm doanh thu trạm mới</button></a>
      ...
      <button onclick="location.href = 'lichsunhap.htm'"  class="btn btn-outline-success my-2 my-sm-0" type="submit">Bỏ lọc</button>
				...
				<c:if test="${message==1}">
					...
					<div class="alert alert-success fade show" role="alert"
						...
						<p>Sửa doanh thu trạm thành công</p>
						<hr>
					</div>
				</c:if>
				<c:if test="${message==2}">
					...
					<div class="alert alert-danger" role="alert"
						...
						<p>Sửa doanh thu trạm thất bại</p>
						<hr>
					</div>
				</c:if>
...
<h2>Lịch sử nhập dữ liệu doanh thu</h2>
					...
					<table class="table">
						<thead>
							<tr>
								<th scope="col">STT</th>
								<th scope="col">Tên trạm vé</th>
								<th scope="col">số vé tập</th>
								<th scope="col">Xử lý</th>
							</tr>
						</thead>
						<tbody>
						<c:set var="counter" value="${offset+1 }" />
							<c:forEach var="doanhThuTram" items="${doanhThuTrams}">
								<tr>
									<th scope="row">${counter}</th>
									<td>${doanhThuTram.tramVe.ten}</td>
									<td scope="row">${doanhThuTram.soVeTap}</td>
									...
									<a
										href="suadoanhthutram/${doanhThuTram.id}.htm"><button
												class="btn btn-primary">chỉnh sửa</button> </a> <!-- 		<button type="button" class="btn btn-danger"
												style='margin-left: 7px;'>xóa</button> -->

										<button
											onclick="if(confirm('bạn có chắc chắn muốn xóa doanh thu này không ?')){location.href='xoadoanhthutram/${doanhThuTram.id}.htm'}"
