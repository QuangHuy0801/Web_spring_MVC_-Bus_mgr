...
<body>
  ...
  <div class="collapse navbar-collapse" id="navbarTogglerDemo01">
   <c:if test="${sessionScope.TaiKhoan.nhanVien.chucVu.id==1}"> <a class="navbar-brand " href="themgiave.htm"> <button type="button" class="btn btn-success"
																data-dismiss="modal">Thêm giá vé mới</button></a></c:if>
...
<h2>Danh sách giá vé</h2>
					...
					<table class="table">
						<thead>
							<tr>
								<th scope="col">STT</th>
								<th scope="col">Giá vé HSSV</th>
								<th scope="col">Giá vé BT</th>
								<th scope="col">Giá vé Tập</th>
						...
						<c:set var="counter" value="${offset+1 }" />
							<c:forEach var="giave" items="${giaves}">

								<tr>
									<th scope="row">${counter}</th>
									<td>${giave.giaVeHSSV}</td>
									<td>${giave.giaVeBT}</td>
									<td>${giave.giaVeTap}</td>
									
									<td>
									<c:if test="${giave.loaiHinh.trim().toLowerCase()=='tg'}">
									Trợ giá
									</c:if>
									<c:if test="${giave.loaiHinh.trim().toLowerCase()=='ktg'}">
									Không trợ giá
									</c:if>
									</td>
									...
									<c:if test="${sessionScope.TaiKhoan.nhanVien.chucVu.id==1}"><a
										href="suagiave/${giave.id}.htm"><button
												class="btn btn-primary">chỉnh sửa</button> </a> <!-- 		<button type="button" class="btn btn-danger"
												style='margin-left: 7px;'>xóa</button> -->

										<button
											onclick="if(confirm('bạn có chắc chắn muốn xóa giá vé này không ?')){location.href='xoagiave/${giave.id}.htm'}"
...
">
			    ...
			    <li class="page-item ${currentPage == 0 ? 'disabled' : ''}">
			        <a class="page-link" href="dsgiave.htm?page=${currentPage - 1}">Trước</a>
			    </li>
			    <c:forEach begin="0" end="${totalPages - 1}" var="i">
			        <li class="page-item ${currentPage == i ? 'active' : ''}">
			            <a class="page-link" href="dsgiave.htm?page=${i}">${i + 1}</a>
			        </li>
			    </c:forEach>
			    <li class="page-item ${currentPage == totalPages - 1 ? 'disabled' : ''}">
			        <a class="page-link" href="dsgiave.htm?page=${currentPage + 1}">Sau</a>
			    </li>
			</ul>
			</div>
				</div>
