...
<body>
  ...
  <div class="collapse navbar-collapse" id="navbarTogglerDemo01">
    <c:if test="${sessionScope.TaiKhoan.nhanVien.chucVu.id==1}"><a class="navbar-brand " href="themtramxe.htm"> <button type="button" class="btn btn-success"
																data-dismiss="modal">Thêm trạm Xe</button></a></c:if>
    <ul class="navbar-nav mr-auto mt-2 mt-lg-0">
      <li class="nav-item active">
       
      </li>
    </ul>
    <form action = "dstramxe.htm" class="form-inline my-2 my-lg-0">
...
" class="btn btn-outline-success my-2 my-sm-0" type="submit">Tìm kiếm</button>
      ...
      <button onclick="location.href = 'dstramxe.htm'"  class="btn btn-outline-success my-2 my-sm-0" type="submit">Bỏ lọc</button>
  </div>
</nav>
				<c:if test="${message==1}">
						...
						<c:set var="counter" value="${offset+1 }" />
							<c:forEach var="tramxe" items="${tramxes}">
								...
								<tr>
									...
									<td><h4>${tramxe.ten}</h4>
									<h6>${tramxe.diaChi}</h6>
									<h6>Nhân viên cập nhật: ${tramxe.nhanVienTaoTramXe.ho} ${tramxe.nhanVienTaoTramXe.ten}</h6></td>
									<td>
									
									<c:if test="${sessionScope.TaiKhoan.nhanVien.chucVu.id==1}"><a
										href="suatramxe/${tramxe.maSo}.htm"><button
												class="btn btn-primary">chỉnh sửa</button> </a> <!-- 		<button type="button" class="btn btn-danger"
												style='margin-left: 7px;'>xóa</button> -->

										<button
											onclick="if(confirm('bạn có chắc chắn muốn xóa trạm xe này không ?')){location.href='xoatramxe/${tramxe.maSo}.htm'}"
...
">
			    ...
			    <li class="page-item ${currentPage == 0 ? 'disabled' : ''}">
			        <a class="page-link" href="dstramxe.htm?page=${currentPage - 1}">Trước</a>
			    </li>
			    ...
			    <c:forEach begin="${x}" end="${y}" var="i">
			        <li class="page-item ${currentPage == i ? 'active' : ''}">
			            <a class="page-link" href="dstramxe.htm?page=${i}">${i + 1}</a>
			        </li>
			    </c:forEach>
			    <li class="page-item ${currentPage == totalPages - 1 ? 'disabled' : ''}">
			        <a class="page-link" href="dstramxe.htm?page=${currentPage + 1}">Sau</a>
			    </li>
			</ul>
			</div>
				</div>
