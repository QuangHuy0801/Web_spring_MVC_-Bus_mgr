...
public class xeBuytController {
	...
	public String themxe(HttpServletRequest request, ModelMap model) {
		Xe xe = new Xe();
		xe.setTinhTrang(1);
		xe.setNhanVienLaiXe(getNhanVienLaiXe().get(0));
		xe.setNhanVienPhuXe(getNhanVienPhuXe().get(0));
		model.addAttribute("nhanVienLaiXes", getNhanVienLaiXe());
		model.addAttribute("nhanVienPhuXes", getNhanVienPhuXe());
	...
	public String suaxe(ModelMap model, @PathVariable String bienSo) {
		Xe xe=this.searchXe(bienSo);
		
		List<NhanVien> list= getNhanVienLaiXe();
		List<NhanVien> list1= getNhanVienPhuXe();
		list.add(xe.getNhanVienLaiXe());
		list1.add(xe.getNhanVienPhuXe());
		model.addAttribute("nhanVienLaiXes",list );
		model.addAttribute("nhanVienPhuXes",list1 );
		model.addAttribute("tuyenduongs", getTuyenDuongs());
	...
	public String updatexe(@ModelAttribute("xe") Xe xe, ModelMap model, BindingResult errors) {
		...
		List<NhanVien> list= getNhanVienLaiXe();
		List<NhanVien> list1= getNhanVienPhuXe();
		list.add(searchNhanVien(xe.getNhanVienLaiXe().getMaNV()));
		list1.add(searchNhanVien(xe.getNhanVienPhuXe().getMaNV()));
		model.addAttribute("nhanVienLaiXes",list );
		model.addAttribute("nhanVienPhuXes",list1 );
		model.addAttribute("tuyenduongs", getTuyenDuongs());
		model.addAttribute("xe", xe);
	...
	public String themtramve(HttpServletRequest request, ModelMap model) {
		...
		model.addAttribute("nhanVienBanVes", getNhanVienTramVe());
	...
	public String suatramve(ModelMap model, @PathVariable int id) {
		...
		List<NhanVien> list= getNhanVienTramVe();
	...
	public String themtuyenduong(HttpServletRequest request, ModelMap model) {
		...
		tuyenDuong.setGiaVe(getGiaVes().get(0));	
		model.addAttribute("tuyenDuong", tuyenDuong);
		model.addAttribute("giaves", getGiaVes());
...
int pageSize = 6; // Số lượng phần tử trên mỗi trang
		...
		int totalUsers = tuyenDuong.getXes().size(); // Tổng số lượng phần tử trên toàn bộ danh sách
			...
			model.addAttribute("message", message);
			model.addAttribute("xes", xes);
			model.addAttribute("tenTuyen", tuyenDuong.getTen());
...
int pageSize = 9; // Số lượng phần tử trên mỗi trang
	...
	@RequestMapping(value="/dsxetrongtuyen/themxetrongtuyen/{id}")
	public String themxetrongtuyen(HttpServletRequest request,ModelMap model,@PathVariable int id,@ModelAttribute("message") String message,@RequestParam(defaultValue = "0") int currentPage) {
		TuyenDuong tuyenDuong= new TuyenDuong();
		tuyenDuong=searchTuyenDuong(id);
		
		List<Xe> listDDL= new ArrayList<Xe>();
		
		
		
		List<Xe> XeConLai = getXeChuaSuaDung(id);
		
		
		int pageSize = 6; // Số lượng phần tử trên mỗi trang
		int totalCount = XeConLai.size(); // Tổng số lượng phần tử trên toàn bộ danh sách
		 int totalPages = (int) Math.ceil((double) totalCount / pageSize);
		 System.out.println(totalCount);
		 if(totalPages==0) {
				totalPages=1;
		...
		System.out.println(endIndex);
		 ...
		 List<Xe> result = XeConLai.subList(startIndex, endIndex);
		...
		model.addAttribute("idTD",id);
		model.addAttribute("xes",result);
		return"tuyenduong/themxetrongtuyen";
	}
	
	@RequestMapping(value="dsxetrongtuyen/themxetrongtuyen/insertxetrongtuyen", method = RequestMethod.POST)
	public String insertxetrongtuyen(ModelMap model ,@ModelAttribute("bienSo") String bienSo,@ModelAttribute("idTD") String idTD) {
		Xe xe= this.searchXe(bienSo);
		TuyenDuong tuyenDuong =this.searchTuyenDuong( Integer.parseInt(idTD));
		xe.setTuyenDuong_xe(tuyenDuong);
		...
		Session session = factory.openSession();
			...
			return "redirect:/dsxetrongtuyen/themxetrongtuyen/" + id1 + ".htm";
	...
	public List<Xe> getXeChuaSuaDung(int id) {
		Session session = factory.getCurrentSession();
		String hql = "FROM Xe where tuyenDuong_xe.id !=:id";
		Query query = session.createQuery(hql);
		query.setParameter("id", id);
		List<Xe> list = query.list();
		return list;
	...
	public List<GiaVe> getGiaVes() {
		...
		public List<NhanVien> getNhanVienTramVe() {
		Session session = factory.getCurrentSession();
		...
		public List<NhanVien> getNhanVienLaiXe() {
			Session session = factory.getCurrentSession();
			String hql = "FROM NhanVien nv WHERE nv.chucVu.id = 3 AND nv.maNV Not IN (SELECT x.nhanVienLaiXe.maNV FROM Xe x)"
					;
			Query query = session.createQuery(hql);
		...
		public List<NhanVien> getNhanVienPhuXe() {
			Session session = factory.getCurrentSession();
			String hql = "FROM NhanVien nv WHERE nv.chucVu.id = 5 AND nv.maNV Not IN (SELECT x.nhanVienPhuXe.maNV FROM Xe x)"
					;
			Query query = session.createQuery(hql);
	...
	public List<Xe> getXestrongtuyen(int page, int pageSize, int idTD,String ten) {
		...
		{
			hql ="FROM Xe t where t.bienSo LIKE CONCAT( '%',:ten, '%') and  t.tuyenDuong_xe.id =:idTD ORDER BY t.bienSo ASC";
			query = session.createQuery(hql);
			int offset = page * pageSize;
			query.setParameter("idTD", idTD);
			query.setParameter("ten", ten);
